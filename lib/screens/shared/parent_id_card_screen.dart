...
class ParentIdCardScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Virtual Parent ID'),
        ...
        actions: [
          IconButton(icon: const Icon(Icons.fullscreen, color: AppColors.textOnDark),
            onPressed: () => _showFullScreen(context, parent, student)),
        ],
      ),
      body: Center(
        child: Padding(
          padding: const EdgeInsets.all(24),
          child: _buildIdCard(parent, student),
        ),
      ),
    );
  }

  Widget _buildIdCard(dynamic parent, dynamic student) {
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Container(
          ...
          child: const Text('PARENT ID CARD', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 12, letterSpacing: 1, color: AppColors.primaryDark)),
        ),
        const SizedBox(height: 20),
        // Parent photo
        CircleAvatar(
          ...
          child: Text(parent.name[0], style: const TextStyle(fontSize: 32, fontWeight: FontWeight.w700, color: AppColors.accent)),
        ),
        const SizedBox(height: 12),
        Text(parent.name, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
        ...
        // QR Code placeholder (use qr_flutter in production)
        Container(
          ...
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            const SizedBox(height: 4),
            Text(parent.qrCodeId, style: const TextStyle(fontSize: 8, color: AppColors.textSubtle)),
          ]),
        ),
        const SizedBox(height: 14),
        Text('ID: ${parent.parentId}', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.textSecondary)),
        const SizedBox(height: 4),
  ...
  void _showFullScreen(BuildContext context, dynamic parent, dynamic student) {
